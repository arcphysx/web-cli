<template>
    <b-button-group class="ml-2 overflow-auto" style="max-width: 80vw;">
        <b-button :variant="activeSession.id == v.id ? 'light' : 'outline-light'" class="text-nowrap" v-for="(v,k) in sessions" :key="k">
            <router-link tag="span" :to="{path: `/tab/${k}`}" class="mr-2">{{ v.name }}</router-link>
...
export default {
    ...
    created(){
        this.switchActiveSession({id: this.$route.params.id})
    },
    methods: {
        ...mapActions(["addSession", "deleteSession", "switchActiveSession"]),
        makeSession(){
            this.addSession();
            this.$router.push(`/tab/${this.activeSessionIndex}`).catch(()=>{})
        },
        removeSession(id){
